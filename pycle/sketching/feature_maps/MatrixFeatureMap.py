...
class MatrixFeatureMap(FeatureMap):
    ...
    def __init__(self, f, Omega, **kwargs):
        ...
        if type(Omega) == tuple or type(Omega) == list:
            ...
            self.bool_multiple_sigmas = (isinstance(self.SigFact, np.ndarray) or isinstance(self.SigFact, torch.Tensor)) and self.SigFact.ndim == 1
            assert self.bool_sigfact_a_matrix or self.bool_multiple_sigmas or isinstance(self.SigFact, numbers.Number)
