...
class CLOMP(Solver):
    ...
    def __init__(self, Phi, K, d_atom, bounds, dfo=False, nb_sample_points=None,  sketch=None, sketch_weight=1., verbose=0):
        """
        - Phi: a FeatureMap object
    ...
    def get_gradient_estimate(self, theta, obj_fun):
        """
        ...
        # ax.scatter(sample_points_X[:, 0], sample_points_X[:, 1], fct_values_Y, c="blue")
        # ax.scatter(theta[0], theta[1], obj_fun(theta.reshape(1, -1)), c="red")
        # ax.set_xlabel('X Label')
        # ax.set_ylabel('Y Label')
        # ax.set_zlabel('Objectif')
        # # plt.show()
        # z = clf.predict(sample_points_X)
        # ax.scatter(sample_points_X[:, 0], sample_points_X[:, 1], z, c="grey")
        # plt.show()

        return lin_param
        # todo compare function evaluation with all function evaluation given by the model and update the
        # radius ball accordingly
    ...
    def _maximize_atom_correlation_fun_grad_dfo(self, theta):
        ...
        # Firstly, compute A(P_theta)...
        sketch_theta, jacobian_theta = self.sketch_of_atom(theta, return_jacobian=True)
        # todo remove this jacobian calculation -> useful only for oracle
        ...
        grad = self.get_gradient_estimate(theta, fun)

        # todo remove this grad computation
    ...
    def _minimize_cost_from_current_sol_dfo(self, p):
        ...
        # todo remove the jacobian calculation
...
def _ThetasToGMM(task, Th, al):
    """util function, converts the output of CL-OMPR to a (weights,centers,covariances)-tuple (GMM encoding in this
    notebook) """
    (K, d2) = Th.shape
    if task == "gmm":
        d = d2 // 2
...
def _CLOMPR_step1_fun_grad(task, Phi, theta, r, z_to_ignore, verbose=0):
    ...
    if np.isclose(Apth_norm, 0):
        if verbose > 1:
            print('ApthNrm is too small ({}), change it to 1e-6.'.format(Apth_norm))
        Apth_norm = 1e-6
    ...
    grad = -np.real(jacobian @ np.conj(r)) / Apth_norm + np.real(
        np.real(jacobian @ Apth.conj()) * (Apth @ np.conj(r)) / (Apth_norm ** 3))
...
def _CLOMPR_step5_fun_grad(task, Phi, p, sketch, z_to_ignore):
    ...
    for i in range(nbthetas):
        ...
        grad[i * thetadim:(i + 1) * thetadim] = -2 * alpha[i] * np.real(
            jacobian_i @ np.conj(r))  # Gradient of the atoms
...
# =========================
def CLOMPR(task, sketch, featureMap, K, bounds, dimensions_to_consider=None, nb_cat_per_dim=None, nIterations=None,
           nRepetitions=1, ftol=1e-6, verbose=0):
    """
    ...
    for iRun in range(nRepetitions):
        ...
        for i in range(nIterations):
            ...
            if task == "kmeans":
                ...
                raise NotImplementedError("There is an error in this code")
...
def CL_GaussianSplitting(task, sketch, featureMap, K, bounds, dimensions_to_consider=None, nb_cat_per_dim=None,
                         nIterations=None, nRepetitions=1, ftol=1e-6, verbose=0):
    """
    """
    ## 0) Defining all the tools we need
    ####################################
...
def CL_histogram_MMD(sketch, Phi, domain, dimension, nb_cat_per_dim=None, bins_cont=10):
    ...
    # Solve
    sol = minimize(_f_grad, x0, method='trust-constr', bounds=None, constraints=constr, jac=True,
                   options={'verbose': 0})
...
def histogramFromSketch_M2M(sketch, Phi, domain, dimension, nb_cat_per_dim=None, bins_cont=10,
                            project_on_probabilitySimplex=True, reg_rho=0.01):
    """
...
if __name__ == "__main__":
    X_sphere = sample_ball(1, 10000)
    print(X_sphere.shape)
    plt.scatter(X_sphere[:, 0], X_sphere[:, 1], s=0.1)
